package guru.qa.rococouserdata.service;
...
public class SmallPhoto {
    ...
    public @Nullable byte[] bytes() {
        if (photo != null) {
            try {
                LOG.info("SmallPhoto processing started");
                LOG.info("Input photo length: {}", photo.length());
                ...
                try (ByteArrayInputStream is = new ByteArrayInputStream(Base64.getDecoder().decode(base64Image));
                    ...
                    byte[] result = concatArrays(
                            ("data:image/" + outputFormat + ";base64,").getBytes(StandardCharsets.UTF_8),
                            Base64.getEncoder().encode(os.toByteArray())
                    );

                    LOG.info("Result photo length: {}", result.length);
                    return result;
                }
            } catch (Exception e) {
                LOG.error("### Error while resizing photo", e);
                throw new RuntimeException(e);
            }
        }
        LOG.warn("SmallPhoto: photo is null");
        return null;
    }
