...
Кроме того, у тебя есть и набор моков для [Wiremock](https://wiremock.org/docs/standalone/docker/) - благодаря этому, 
даже не приступая к написанию кода, ты сможешь посмотреть механику проекта Rococo. 
В этом наборе моков есть все запросы, которые тебе в конце концов, придется реализовать в своем бэкенде.
Важно понимать, что несмотря на наличие моков mutation запросов (например, удаление картины), 
никакого реального удаления не произойдет, и при обновлении страницы
...
Тестовое покрытие niffler, которого мы с тобой добились на настоящий момент, однако, является достаточно слабым - 
  учтите это при написании тестов на Rococo - это,
...
Dmitriis-MacBook-Pro rangiffler-client % cd rococo-auth
...
```
  Эти команды надо запускать в корне проекта, там же есть скрипт `wiremock.sh`, делающий ровно то же самое. 
  Можно просто запустить его.

  Wiremock стартанет на порту 8080: http://localhost:8080/ и будет готов отдавать тебе статические ответы на все запросы,
  уходящие с фронта rococo. Кнопка "Войти" тоже работает, ведь у нас запущен сервис auth.
Наш Wiremock понимает некоторые запросы с oauth token, при чем ему не важно - какой именно это токен.
Поэтому, что бы смоделировать ситуация "я залогинился", можно просто зайти в dev tools браузера,
...
Само собой, ты можешь просто сразу залогиниться через сервис auth.
...
- Контроллеры
- Сервисы, которые на первых порах могут и должны возвращаться просто замоканные данные в контроллеры (т.е. мы по сути 
пишем свой wiremock на спринге, для начала)
...
###### Сервис gateway, работа с пагинацией
   ...
   Проект Rococo использует пагинацию (что бы грузить данные с бэка по частям), а это значит что в `rococo-gateway` должны быть реализованы 
Pageble контроллеры.
Пример:
```java
  @GetMapping()
...
#### 8. Реализовать запуск тестов в CI pipeline GHA

  И загрузку Allure в учебный Allure Server или GithubPages

#### 9. Оформить все красиво!
